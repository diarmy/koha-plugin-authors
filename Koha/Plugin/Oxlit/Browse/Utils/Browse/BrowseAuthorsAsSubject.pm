package Koha::Plugin::Oxlit::Browse::Utils::Browse::BrowseAuthorsAsSubject;
...
sub listBrowseResults {
    ...
    my @countParams = ();
    my $countSql = "SELECT COUNT(DISTINCT author) FROM oxlit_authorsassubject";
    if ($starts_with) {
        $countSql .= " WHERE author LIKE ?";
    ...
    my $sql = "SELECT DISTINCT author FROM oxlit_authorsassubject";
    my @params = ();
    ...
    $sql .= " ORDER BY REGEXP_REPLACE(author, '[^A-Za-z]', '') LIMIT ? OFFSET ?";
