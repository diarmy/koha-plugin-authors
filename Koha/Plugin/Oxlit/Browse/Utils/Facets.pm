package Koha::Plugin::Oxlit::Browse::Utils::Facets;
...
( undef, $results_hashref, \@facets_loop ) = getRecords (
        ...
        $koha_query,       $facet_config, $sort_by_ref,    $servers_ref,
    ...
    _ZOOM_event_loop(
        ...
        sub {
            ...
            if ( !$scan && $servers[ $i - 1 ] =~ /biblioserver/ ) {
                $facets_counter = GetFacets( $results[ $i - 1 ], $facet_config );
...
sub GetFacets {
    ...
    my ($rs, $facet_config) = @_;
    my $facets;

    $facets = _get_facets_from_zebra($rs, $facet_config);
...
=head2 _get_facets_from_zebra
    ...
    my $facets = _get_facets_from_zebra( $result_set, $facet_idx );
...
sub _get_facets_from_zebra {
    ...
    my $facets_data = {};
    # use Data::Dumper;
    # warn "FACETS LOOP: ".Dumper($facets_loop);

    my $facet_values = _get_facet_from_result_set( $facet_config, $rs);
    if ($facet_values) {
        $facets_data->{$facet_config->{idx}} = $facet_values;
    }
...
=head2 _get_facet_from_result_set
    ...
    my $facet_values =
        C4::Search::_get_facet_from_result_set( $facet_config, $result_set )
...
sub _get_facet_from_result_set {
    ...
    my $facet_config = shift;
    my $rs           = shift;

    my $internal_sep  = '<*>';
    my $sep = $facet_config->{sep} // ', ';
    my $idx = $facet_config->{idx} // '';
    my $facet_max_count = $facet_config->{max_count} // 20;

    return if ( !defined $facet_config || !defined $rs );

    # zebra's facet element, untokenized index
    my $facet_element = "zebra::facet::$idx:$facet_max_count";
